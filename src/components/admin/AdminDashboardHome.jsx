...
const getStatusText = (batch) => {
  if (batch.quantity > 0 && dashFormatExpiry(batch.expiryDate).startsWith("Expired")) {
  ...
  const dynamicStats = adminStats.map((stat) => {
    ...
    if (stat.label === "Active Alerts") {
      ...
      return {
        ...
        sub: dashLoading ? "Loading..." : alertCount === 0 ? "No urgent batches" : "Review at-risk inventory",
      };
    }
        ...
        <CardHeader className="p-8 border-b border-[#0F172A]/5">
          <div>
            ...
            <p className="mt-1 text-sm font-bold text-[#0F172A]/55">
              Snapshot of batches that need quick attention on the admin dashboard.
            </p>
          </div>
        </CardHeader>
                  ...
                  return (
                    ...
                    >
                      ...
                      <TableCell className="text-right px-8">
                        <span className="text-xs font-black uppercase tracking-wider text-[#0F172A]/55">
                          {getStatusText(batch)}
                        </span>
                      </TableCell>
                    </TableRow>
